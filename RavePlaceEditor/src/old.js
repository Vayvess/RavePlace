...
export default class Editor extends Phaser.Scene {
    ...
    spawnRaveNode(text) {
        ...
        const worldX = camera.midPoint.x;
        const worldY = camera.midPoint.y;

        // Snap to grid
        const snappedX = Math.round(worldX / gridSize) * gridSize;
        const snappedY = Math.round(worldY / gridSize) * gridSize;

        const node = new RaveNode(this, snappedX, snappedY, text);
    ...
    create() {
        ...
        this.highlight = this.add.rectangle(0, 0, 64, 64, 0x3498db, 0.2).setStrokeStyle(2, 0x3498db);
    ...
    update(time, delta) {
        ...
        const worldX = cam.midPoint.x;
        const worldY = cam.midPoint.y;
